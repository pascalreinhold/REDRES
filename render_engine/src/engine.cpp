...
void Engine::processMouseDrag() {
  ...
  offset[0] = (current_pos[0] - last_pos[0])/window_->width()* 2. * glm::pi<double>();
  offset[1] = (current_pos[1] - last_pos[1])/window_->height()* 2. * glm::pi<double>();
...
void Engine::render() {
...

  {
    ...
    vk::Result submit_result = graphics_queue_.submit(1, &submit_info, getCurrentFrame().render_fence);
...
void Engine::GetOptimalCameraPerspective() {
  ...
  Eigen::JacobiSVD<Eigen::MatrixXf> svd(transformedPositions, Eigen::ComputeThinV | Eigen::ComputeThinU);
