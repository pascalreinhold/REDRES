...
Eigen::Vector3f VisualizationData::calcMicDisplacementVec(const Eigen::Vector3f &pos1,
                                                          const Eigen::Vector3f &pos2) const {
        ...
        for (uint32_t j = 0; j < framePositions.rows(); j++) {
            for (uint32_t k = j + 1; k < framePositions.rows(); k++) {
                ...
                if (squaredDistance < cutOff) {
                    ...
                    if (squaredDistance < squaredFudgeFactor*(elmInfo1.atomRadius + elmInfo2.atomRadius)) {
                        const glm::vec3
                            pos1{framePositions.row(j)(0), framePositions.row(j)(1), framePositions.row(j)(2)};
