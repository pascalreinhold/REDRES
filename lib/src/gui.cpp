...
void UserInterface::showEventsTable(int experimentID) {
    if (ImGui::BeginTable("EventsTable", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
        ...
        for (int j = 0; j < events.size(); j++) {
            ...
            bool isActive = false;
            if (parentEngine->scene_->visManager->data().activeEvent)
                isActive = std::get<0>(events[j])
    ...
    if (ImGui::Begin("Event Info", nullptr, ImGuiWindowFlags_AlwaysUseWindowPadding)) {
        if (ImGui::BeginChild("Event Info Child")) {
            ...
            if (ImGui::Button("Leave Event Mode")) {
                ...
                for (const auto
                        &atomIndex : parentEngine->scene_->visManager->data().activeEvent->chemical_atom_numbers) {
                    ImGui::Text("Atom Number: %d", atomIndex);
                }
                for (const auto
                        &atomIndex : parentEngine->scene_->visManager->data().activeEvent->catalyst_atom_numbers) {
                    ImGui::Text("Atom Number: %d", atomIndex);
                }
                ...
                ImGui::Text("%s",
                            glm::to_string(parentEngine->scene_->visManager->data().activeEvent->surfaceNormal).c_str());
        ...
        if (ImGui::Begin("Left Aligned",
            ...
            if (ImGui::BeginChild("Infos:", {0.f, 0.f}, false, ImGuiWindowFlags_AlwaysUseWindowPadding)) {
                if (!parentEngine->camera_->is_isometric)
                    ImGui::InputFloat3("Camera Coords",
                                       reinterpret_cast<float *>(&parentEngine->camera_->position_));
                ImGui::InputFloat3("View Direction",
                                   reinterpret_cast<float *>(&parentEngine->camera_->view_direction_));
            ...
            if (parentEngine->experiment_state_!=eNone) {
                ...
                if (parentEngine->ui_mode_==uiMode::eSelectAndTag) {
                    ...
                    if (ImGui::Button("Tag Selection as Chemical (to DB)")) {
                        ...
                        parentEngine->scene_->visManager->updatePropertyForSelectedAtomsToDB(experimentID,
                                                                                             baseTypePropertyID,
                                                                                             chemicalID);
                    }
...
#endif
                if (parentEngine->scene_->freezeAtom()!=-1)
                    ImGui::Text("Freeze AtomID: %d",
                                parentEngine->scene_->visManager->data().atomIDs[parentEngine->scene_->freezeAtom()]);
                ...
                if (!parentEngine->selected_atom_numbers_.empty()) {
                    if (parentEngine->selected_atom_numbers_.size()==1) {
                        ...
                        Eigen::Vector3f pos1 =
                            parentEngine->scene_->visManager->data().positions[parentEngine->GetMovieFrameIndex()].row(
                                index1);
                        uint32_t index2 = parentEngine->selected_atom_numbers_[1];
                        Eigen::Vector3f pos2 =
                            parentEngine->scene_->visManager->data().positions[parentEngine->GetMovieFrameIndex()].row(
                                index2);
                        Eigen::Vector3f displacement21 =
                            parentEngine->scene_->visManager->data().calcMicDisplacementVec(pos2, pos1);
                        ...
                        if (parentEngine->selected_atom_numbers_.size()==3) {
                            ...
                            Eigen::Vector3f pos3 =
                                parentEngine->scene_->visManager->data().positions[parentEngine->GetMovieFrameIndex()].row(
                                    index3);
                            Eigen::Vector3f displacement23 =
                                parentEngine->scene_->visManager->data().calcMicDisplacementVec(pos2, pos3);
