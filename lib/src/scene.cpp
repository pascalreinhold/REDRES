...
std::string AtomType::ObjectInfo(uint32_t movieFrameIndex, uint32_t inTypeIndex) const {
    ...
    std::string
        symbol = s.visManager->data().elementInfos.find(s.visManager->data().tags[inTypeIndex] & 255)->second.symbol;
...
uint32_t BondType::MaxCount() const {
    const auto
        largestVectorElement = std::max_element(s.visManager->data().bonds.begin(), s.visManager->data().bonds.end(),
...
void UnitCellType::writeToObjectBufferAndIndexBuffer(uint32_t movieFrameIndex,
    ...
    objectSSBO[object_index].color1 =
        (selectedObjectIndex!=s["Atom"].Count(movieFrameIndex)) ? glm::vec4(1.f) : glm::vec4(0.f, 1.f, 0.f, 1.f);
...
void VectorType::writeToObjectBufferAndIndexBuffer(uint32_t movieFrameIndex,
    ...
    for (int i = 0; i < s.visManager->data().hinuma_atom_numbers.size(); i++) {
        ...
        auto const
            &elm_info = s.visManager->data().elementInfos.find(s.visManager->data().tags(id_vec[i]) & 255)->second;
        ...
        objectSSBO[object_index].radius =
            s.meshes->meshInfos[meshID::eVector].radius*length*s.gConfig.hinumaVectorLength;
