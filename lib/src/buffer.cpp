...
uint32_t ResourceManager::createBuffer(size_t size,
    ...
    vk::BufferCreateInfo
        buffer_create_info{vk::BufferCreateFlags(), size, buffer_usage, sharing_mode, queue_family_indices};
...
void ResourceManager::writeToBuffer(BufferResource buffer_resource, const void *data, vk::DeviceSize size) {
    ...
    void *final_location =
        reinterpret_cast<char *>(buffer.mapped_data_) + buffer_resource.descriptor_buffer_info_.offset;
    memcpy(final_location, data, size);
}

void ResourceManager::immediateSubmit(UploadContext &upload_context,
...
void ResourceManager::stageBuffer(void *src,
                                  vk::DeviceSize size,
                                  BufferResource dest_resource,
    ...
    BufferResource staging_buffer_resource =
        createBufferResource(staging_buffer_handle_, 0, size, vk::DescriptorType::eStorageBuffer);
...
void ResourceManager::clearBuffer(BufferResource buffer_resource) {
    ...
    memset(mapped,
           static_cast<int>(buffer_resource.descriptor_buffer_info_.offset),
           buffer_resource.descriptor_buffer_info_.range);
}

std::pair<BufferResource, BufferResource> ResourceManager::uploadMesh(Mesh &mesh,
