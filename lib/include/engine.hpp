...
class Engine {
  ...
  private:
    ...
    // buffers
    FrameData frame_data_[Swapchain::MAX_FRAMES_IN_FLIGHT];
    ...
    vk::PipelineLayout deferred_pipeline_layout_;

    std::unique_ptr<Pipeline> atom_pipeline_, bond_pipeline_, deferred_pipeline_;
    vk::DescriptorSetLayout graphics_descriptor_set_layout, deferred_descriptor_set_layout;
    ...
    vk::Pipeline createComputePipeline(const std::string &shader_path,
    ...
    void beginPresentRenderPass(vk::CommandBuffer &cmd, uint32_t swapchain_index);
    ...
    void beginOffscreenRenderPass(vk::CommandBuffer &cmd);
};
